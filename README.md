...
- farm GPS, climate baseline and short-term weather screen
...
2. farm GPS, automatic long-term climate and 7-day weather
...
- Browser Geolocation API for farmer-controlled farm coordinates
...
## Farm GPS, climate and weather

FarmCompass does not ask farmers to guess average rainfall or temperature. On the **My Farm** screen the farmer can tap **Use my current location**. The browser/device Geolocation API returns latitude, longitude and an accuracy estimate after the farmer grants permission.

When the profile is saved with coordinates, FarmCompass requests a long-term climate baseline from the Open-Meteo Historical Weather API. The implementation uses the most recent 20 complete calendar years of ERA5 reanalysis and calculates:
...
The two headline climate values are stored on the farmer profile and are automatically used by the crop recommendation engine when it scores crop rainfall and temperature compatibility. Historical climate values are treated as area-level estimates
...
The farmer can remove the saved coordinates from the profile. Administrators can see coordinates and climate fields because they can view the complete farmer profile, but the administrator still cannot see recommendation history or AI conversation history.

**Development note:** browser geolocation requires a secure context. It works on `localhost` during local development and should be deployed over HTTPS for phones and production use. Open-Meteo does not require an API key for the public non-commercial endpoint used by this project.
