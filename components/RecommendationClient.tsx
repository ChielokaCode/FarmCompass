    ...
    {!profile ? <section className="fc-card p-5">
      ...
      {recs.length > 0 && <section>
        ...
        <div className="mt-4 space-y-3">{recs.map((r, i) => <article key={r.crop.slug} className="fc-card p-5">
          ...
          <ul className="mt-4 space-y-2">{r.reasons.slice(0,3).map(reason => <li key={reason} className="flex gap-2 text-sm leading-6 text-slate-600"><span className="mt-1 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-emerald-50 text-emerald-700"><AppIcon name="check" className="h-3 w-3"/></span><span>{reason}</span></li>)}</ul>
